...
const  PricingSection = ({subheading,allSubectPricing,subjectPricing}) => {
  return (
...
>
            {subjectPricing.text} <ArrowRight className="w-4 h-4 ml-2" />
          </Button>
          </Link>
          <Link href={allSubectPricing.url || "#"}>
          <Button
            variant="link"
            className="bg-white text-teal-600 hover:bg-gray-100 font-semibold px-6 py-2 rounded-full shadow-sm transition"
          >
            {allSubectPricing.text} <ArrowRight className="w-4 h-4 ml-2" />
          </Button>
          </Link>
        </div>
      </div>
