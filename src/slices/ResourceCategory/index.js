...
const ResourceCategory = ({ slice ,context}) => {
  ...
  "Study Guides": <Lightbulb className="w-6 h-6 text-teal-600" />,
};
...
{slice.primary.heading
  ? slice.primary.heading
  : `${subject.charAt(0).toUpperCase() + subject.slice(1)} Resources`}
        </h1>
      ...
      <div className="flex justify-center gap-3 mb-14 flex-wrap">
        {categories.map((category) => (
          <Button
            key={category}
            variant={activeTab === category ? "bg-teal-600" : "ghost"}
...
`}
          ...
          >
            ...
            <span>{subject.charAt(0).toUpperCase()+subject.slice(1)+" "+category}</span>
          </Button>
        ))}
      </div>
