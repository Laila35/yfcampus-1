...
const Level1AlevelFocus = ({ slice, context }) => {
    ...
    // Find the index of "Practical Skills and Data Analytics"
    const practicalSkillsIndex = topics.findIndex(topic => 
        topic.toLowerCase().includes("practical skills") || 
        topic.toLowerCase().includes("data analytics")
    );
            ...
            <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-center">
                    {topics.map((topic, index) => {
                        ...
                        const isPracticalSkills = index === practicalSkillsIndex;
                        ...
                        return (
                            <div
                                ...
                                className={`px-4 py-4 rounded-full border border-teal-500 bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-300 font-medium hover:bg-teal-100 dark:hover:bg-teal-800/30 transition-all ${
                                    ...
                                    isPracticalSkills ? "md:col-start-2" : ""
                                }`}
                            >
                                {index + 1}. {topic}
                            </div>
